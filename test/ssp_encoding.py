...
for i in range(n_test):
    ...
    assert args.binding is not None, 'Binding types for each entry is not provided'
    assert args.aggregate is not None, 'Aggregation types between features are not provided'
    dim = args.dim
    ...
    coord_sp = encode_feature(coord_noisy, [x_axis_sp, y_axis_sp, z_axis_sp], binding = args.binding[0], aggregate = args.aggregate)
    kind_sp = encode_feature(kind_noisy, [nut_sp, bolt_sp], binding = args.binding[1], aggregate = args.aggregate)
    anchor_sp = encode_feature(anchor_noisy, [table_sp, jig_sp], binding = args.binding[2], aggregate = args.aggregate)
    if args.with_anchor:
        data_concat = [coord_sp, kind_sp, anchor_sp]
    if args.with_anchor:
        data_concat = [coord_sp, kind_sp]

    data_encoded = encode_dataset(data_concat, args.aggregate_between_feature)
    C = Clustering(data_encoded, args.algorithm, args.affinity, n_clusters, thres = None, p = -2)
